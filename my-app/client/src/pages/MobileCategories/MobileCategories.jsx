...
import SubcategoryProductListing from './components/SubcategoryProductListing';
...
const MobileCategories = () => {
  ...
  const [activeSubcategory, setActiveSubcategory] = useState(null); // { categoryName, subcategoryName }
  const [miniBannerData, setMiniBannerData] = useState(null);

  // Store right panel scroll offset per category ID
  const scrollPositions = useRef({});
  const rightPanelRef = useRef(null);

  // 1. Fetch Categories & Admin Page Content
  useEffect(() => {
    const loadCategoriesData = async () => {
      setLoading(true);
      try {
        const [catRes, bannerRes] = await Promise.all([
          fetch(`${API_BASE}/api/categories/with-counts`),
          fetch(`${API_BASE}/api/page-content/categories`).catch(() => null)
        ]);
  ...
  const handleSelectCategory = (catId) => {
    ...
    setActiveSubcategory(null); // Reset subcategory listing view
  ...
  // 3. Handle subcategory selection -> open product listing screen
  const handleSelectSubcategory = (subName, subObject) => {
    const currentCategory = categories.find(c => c._id === activeCategoryId || c.name === activeCategoryId);
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 flex flex-col font-sans select-none md:hidden">
      ...
      {!activeSubcategory && (
        <header className="bg-white border-b border-slate-200/80 px-4 py-3 sticky top-0 z-30 shadow-3xs flex flex-col gap-2.5">
          {/* Title Row */}
          <div className="flex items-center justify-between">
            <h1 className="text-base font-black text-slate-900 tracking-tight">
              Shop by category
            </h1>
            <div className="w-8 h-8 rounded-full bg-blue-50 text-[#0057FF] flex items-center justify-center">
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full bg-[#F5F7FF] border border-blue-100/70 pl-9 pr-8 py-2 rounded-full text-xs font-medium text-slate-800 placeholder:text-slate-400 outline-none focus:border-[#0057FF] focus:bg-white transition-all shadow-2xs"
            />
            {searchQuery && (
              <button
      ...
      {/* ─── 2. MAIN CONTENT AREA (Full height split or Subcategory listing) ─── */}
      <main className="flex-1 flex overflow-hidden h-[calc(100vh-140px)]">
        {activeSubcategory ? (
          /* Subcategory Product Listing Screen */
